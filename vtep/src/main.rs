...
use carve::{
    config::AppConfig,
    redis_manager::{CompetitionStatus, RedisManager},
};
use redis::Commands;
...
impl NetworkManager {
    ...
    fn create_vxlan_interface(&self, name: &str, vxlan_id: u32) -> Result<()> {
        ...
        let eth0_ip = eth0_ip
            .split('/')
            .next()
            .context("Failed to split eth0 IP address")?;
    ...
    fn manage_nat_rule(&self, enable: bool, rule_active: &mut bool) -> Result<()> {
        ...
        match (enable, *rule_active) {
            (true, false) => {
                self.ipt
                    .append("nat", "POSTROUTING", nat_rule)
            ...
            (false, true) => {
                self.ipt
                    .delete("nat", "POSTROUTING", nat_rule)
...
impl FdbManager {
    async fn update_fdb_entries(config: &AppConfig) -> Result<()> {
        for (comp_idx, competition) in config.competitions.iter().enumerate() {
            let redis_manager =
                RedisManager::new(&competition.redis).context("Failed to create Redis manager")?;
    ...
    async fn publish_fdb_entry(
        ...
        redis_manager
            .create_vxlan_fdb_entry(&competition.name, mac_address, addr.ip(), &team.name)
            .await
    ...
    async fn update_bridge_fdb(
        ...
        let fdb_entries = redis_manager
            .get_domain_fdb_entries(&competition.name, &team.name)
            .await
    ...
    fn add_fdb_entry(vxlan_name: &str, mac: &str, addr: &str, is_broadcast: bool) -> Result<()> {
        ...
        if !status.success() {
            bail!(
                ...
                if is_broadcast { "broadcast" } else { "unicast" },
                status
            );
        }
...
fn setup_competition_network(
    competition: &carve::config::Competition,
    comp_idx: usize,
) -> Result<()> {
    let cidr = competition
        .cidr
        .as_ref()
        .context("competition.cidr missing")?;
    ...
    subnet_map.insert(
        "MGMT".to_string(),
        format!("{}/24,MGMT,0", network_config.mgmt_subnet),
    );
    ...
    // Setup team networks
    for (i, (team, &subnet)) in competition
        .teams
        .iter()
        .zip(&network_config.team_subnets)
        .enumerate()
    {
    ...
    let subnet_pairs: Vec<_> = subnet_map
        .iter()
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();

    let _: () = con
        .hset_multiple(format!("{}:subnets", competition.name), &subnet_pairs)
...
async fn manage_competition_nat(config: &AppConfig) -> Result<()> {
    ...
    let current_state = redis_manager
        .get_competition_state(competition_name)
        .await
...
async fn main() -> Result<()> {
    ...
    for (comp_idx, competition) in config.competitions.iter().enumerate() {
        setup_competition_network(competition, comp_idx).with_context(|| {
            format!(
                "Failed to setup network for competition {}",
                competition.name
            )
        })?;
    }
