use carve::{config::{AppConfig, Competition}, redis_manager::RedisManager};
use actix_web::{
...
async fn get_team(
    ...
    let members = match redis.get_team_users(&competition.name, &team.name) {
        Ok(users) => {
            users.into_iter().map(|user| TeamMember {
                id: user.user_id.unwrap_or(0),
                name: user.username,
            }).collect()
        }
...
async fn get_teams(competition: web::Data<Competition>) -> ActixResult<impl Responder> {
    let teams: Vec<TeamListEntry> = competition.teams.iter().enumerate()
...
async fn get_score(
    ...
    let start_time = query.start_date
        .unwrap_or_else(|| Utc::now() - chrono::Duration::days(1))
        .timestamp_millis();
    let end_time = query.end_date
    ...
    let teams_to_check: Vec<_> = if let Some(team_id) = query.team_id {
        ...
        vec![(team_id, competition.teams[team_id as usize - 1].name.clone())]
    } else {
        competition.teams.iter().enumerate()
...
async fn get_boxes(
    ...
    let boxes: Vec<BoxInfo> = competition.boxes.iter()
        .map(|box_config| BoxInfo {
            name: format!("{}.{}.{}.local", box_config.name, team_name.to_lowercase(), competition.name.to_lowercase()),
        })
        .collect();
...
async fn get_box(
    ...
    let box_exists = competition.boxes.iter()
        .any(|b| b.name == box_type);
    
    // Verify the team exists
    let team_exists = competition.teams.iter()
    ...
    let ip_address = if let Some(ref vtep_host) = competition.vtep_host {
        resolve_box_ip(&query.name, vtep_host).await
...
async fn get_checks(competition: web::Data<Competition>) -> ActixResult<impl Responder> {
    let checks: Vec<CheckResponse> = competition.checks.iter()
...
async fn oauth2_callback() -> ActixResult<impl Responder> {
    // Mock OAuth2 callback - in a real implementation, this would handle OAuth2 flow
...
async fn main() -> std::io::Result<()> {
    ...
    let competition_name = std::env::var("COMPETITION").unwrap_or_else(|_| "default".to_string());
